...
async function trailData(lat, lon) {
  ...
  const mapData = apiResponseData.trails.map((trailObj) => {
    return {
      name: trailObj.name,
      location: trailObj.city,
      length: trailObj.distance,
      stars: trailObj.rating,
      star_votes: trailObj.ratings,
      summary: trailObj.crossstreets,
      trail_url: trailObj.url,
      conditions: trailObj.surface
    };
  });
  return mapData;
}
